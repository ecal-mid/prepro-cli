...
const getDepths_ = (inputFolder, outputFolder, frames, url, params) => {
...
const run = (inputFolder, outputFolder, url, params) => {
  return new Promise((resolve, reject) => {
    ...
    while (frames.length) {
      ...
      const p = getDepths_(inputFolder, outputFolder, slice, url, params);
      promises.push(p);
    }

    const output = path.join(outputFolder, 'depth.mov');
